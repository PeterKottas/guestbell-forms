...
export class TagsRaw extends BaseInput<TagsRawProps, TagsState, HTMLInputElement> {
						...
						!this.props.readOnly && (
							...
							>
								<Text
									...
									className={
										'tags-input__text-input ' + (textProps.className ? textProps.className : '')
									}
									...
									<TagsSuggestions
										...
										AddNewTagComponent={
											...
											(!this.state.fetchedExistingTags ||
												!this.state.fetchedExistingTags.find(e => e.name === this.state.value)) &&
	...
	private onKeyDown = (suggestions: Tag[]) => async e => {
		if (
			e.key === 'Enter' &&
			(this.state.value !== '' || this.state.preselectedSuggestion !== undefined) &&
			this.state.textErrors.length === 0
		) {
			e.preventDefault();
			e.stopPropagation();
			const existingTag =
				this.props.existingTags && this.props.existingTags.find(et => et.name === this.state.value);
	...
	private renderTag(tag: Tag, index: number) {
		return (
			<div className="tags-input__tag" key={index}>
				{tag.name}
				{!this.props.readOnly && (
					<Button
						{...(this.props.id && {
	...
	private renderTags() {
		return this.props.tags && this.props.tags.length > 0 ? (
			...
			this.props.readOnly && (
				<div className="tags-input__tag__wrapper">
					<div className="tags-input__tag">{this.props.readonlyEmptyPlaceholder}</div>
				</div>
			)
		);
	}
