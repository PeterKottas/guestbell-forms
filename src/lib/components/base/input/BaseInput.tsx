...
> extends React.Component<P, S> implements ComponentApi {
	...
	protected renderLabel(touchable: boolean = false) {
		if (!this.props.tooltip || (this.props.tooltip && this.props.title)) {
			return <span className={touchable ? '' : 'label--no-touch'}>{this.props.label}</span>;
		}
		return (
			<React.Fragment>
				<span className={touchable ? '' : 'label--no-touch'}>{this.props.label}</span>
				{this.renderTooltip()}
			</React.Fragment>
		);
	}
			...
			if (!props.required && !value) {
					...
					props.validators.forEach(validator => {
						...
						switch (validator) {
							...
							case 'longitude':
								...
								break;
							case 'url':
								validInner = new Validators.UrlValidator().Validate(value, props.required, error =>
									errors.push(error)
								);
								break;
							default:
				...
				if (props.customValidators) {
					props.customValidators.forEach(customValidator => {
						let validInner = false;
						validInner = customValidator.Validate(value, props.required, error =>
							errors.push(error)
						);
						if (isValid && !validInner) {
							isValid = validInner;
